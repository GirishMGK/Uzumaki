...
| Tool | What it does |
|------|--------------|
| **Parquet Tool** (`parquet_tool.py`) | Python/Streamlit port of the original WPF app: Convert (CSV/Excel→Parquet w/ compression + type inference), Viewer (schema, row-groups, stats, filter, sort), CSV Tools (delimiter, encoding, merge, split-rows, schema-compare), Analytics (DuckDB SQL + query history). |
| **Statutory Extractor** (`Combined_PF_Statutory.py`) | PF Challan / ECR Return / TRRN **and** ESI · PT · TDS · GSTR-1 · GSTR-3B extraction — already combined into one two-tab app (shared PDF-read/regex utilities). Accepts individual PDFs or a ZIP of a whole folder. |
